...
- `docs/PRD.md` — define O QUE é a biblioteca: objetivos, tipos públicos (com assinaturas C# completas), padrões, fluxo de erros e decisões de design. **É a fonte de verdade da API.**
- `docs/DEVELOPMENT_PLAN.md` — define COMO construir: estrutura da solution, `.csproj`, ordem de implementação, matriz de testes, samples, NuGet/CI/CD.
...
`ReturnSuccessOrError` — biblioteca NuGet (.NET 9 / C# 13) para a camada de domínio em Clean Architecture. É inspirada no package Dart homônimo (`C:\PROJETOS\FLUTTER\PACKAGES\return_success_or_error`), mas é um produto **nativo .NET** — não um porte literal. Zero dependências de runtime; AOT-friendly.
...
- Testes: xUnit + NSubstitute (mock de `IDataSource<T>`) + FluentAssertions. Cobrir obrigatoriamente: curto-circuito, `Cod. 02-1`, `Cod. BackgroundCatch`, paridade direto↔background e propagação do `CancellationToken`.
- Sem reflexão em runtime (manter AOT-friendly).
